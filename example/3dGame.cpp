...
void GL3DGame::LoadData()
{
	...
	//a = new Actor(this);
	//a->SetPosition(Vector3(100.0f, 575.0f, 0.0f));
	//a->SetScale(100.0f);
	//q = Quaternion::Concatenate(q, Quaternion(Vector3::UnitY, Math::PiOver2));
	//q = Quaternion::Concatenate(q, Quaternion(Vector3::UnitX, Math::PiOver2));
	////a->SetRotation(q);
	//mc = new MeshComponent(a);
	//mc->SetModel(std::move(Model::Load("Assets/model/backpack.obj")));
	...
	//point light actor
	{
		...
		//		Vector3 pos = Vector3(start + i * size, start + j * size, -50);
		//		a->SetPosition(pos);
